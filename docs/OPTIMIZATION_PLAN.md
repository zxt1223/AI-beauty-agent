...
- [x] **避雷共识补最小样本下限（P0-1 审计修复，2026-09-02）**：外部对抗审计指出 70% 比例无样本下限——单条差评 1/1=100% 即永久避雷（195 缺陷证据商品 78 款进避雷、命中项负面中位仅 1 条）。**用户拍板**：加 `config.DEFECT_MIN_NEG=5`（该商品负面评论 ≥5 才参与 70% 判定）+ 分母注释写死 = 该商品负面评论总数（非含好评全部评论）；旧口径归档 `legacy_consensus_axes()`。**只影响运行时动态 map（78→5 款）+ 未来重建池，现役静态评测池不动** → 三轨零漂移：锚点 94.7%/105/105/24/24/29.2%、v3 四维 26.7%/0.268/16-16/资损 38/39、语义试探 200 条进入 66/通过 14/误放 0，逐数字复现。详见 agent_design.md §8.19
- [x] **整改 2-6：评测文档治理收口（P0-3/P0-4/P4/P0-2/P3-9/P3-10 审计整改，2026-09-02，纯文档零代码）**：**①评测文件治理（P0-3/P4）**——实测 MySQL 真值（eval_review_50=**200 行**/对比表1 仍 41（v3 只进 eval_review_50）/candidate_pool_v2=**10702 行/200 题** v3 扩池），database_schema 陈旧行数 41→200、2249→10702 同步 + 新建 [data/README.md](../data/README.md) 资产真相源清单（文件角色/行数/再生成命令）+ README「eval_review_50.csv 可装载快照」更正为 v1 遗留 11 行样例；**②标签生产链路（P0-4）**——新建 [docs/label_pipeline.md](docs/label_pipeline.md)（五维 schema + 三级来源降权 + coverage 自动 381→人工精标 14→兜底 695 + 覆盖 36.2% + QA/演进）；**③指标口径字典（P4）**——新建 [docs/metrics_glossary.md](docs/metrics_glossary.md)（首答分母 19 的坑/锚点 vs v3 两域/两个「置信度」区分/避雷分母口径/eval_review_50.csv 11 行勿当真值）；**④P0-2 措辞**——置信度三档诚实化（规则固定分+LLM 自评、未统计校准、85/60 工程定标）写入 ARCHITECTURE；**⑤P3-9/P3-10 措辞**——「架构形状≈企业级」改「参考企业级分层」、Harness 补「轻量壳 vs 目标架构」边界、ARCHITECTURE defect_consensus 重复小节删除。锚点零改动
...
- [x] **评测集 v3：三轨扩充 159 条（2026-09-02 完成，对标 C4 评论抽取 + ESCI 相关度）**：评测集 41 → **200 题**（锚点 1-41 隔离 + 新增 id 42-200）。三轨结构：**好评轨 88**（直说/避雷/色号/持妆/质地 各 15 + 预算 6/硬约束 7，primary=评论商品客观口径）/ **模拟轨 32**（16 能推荐进 NDCG + 16 追问测追问决策，分层）/ **差评轨 39**（避雷类，**降级资损断言**——实证 primary 无法自动标：39 题仅 12 题 negative_axes 识别、弱约束空壳 top-1、defect 表 15/39 无轴，不配 primary 不算 NDCG，断言首答不含被吐槽商品）。**四维指标（_eval_v3.py）**：首答 **20/75=26.7%** / NDCG@5 0.268 / Recall@5 0.333 / MRR 0.271 / 追问 16/16=100% / 资损 38/39=97.4%（唯一泄漏 B01LZUPT3C=defect 表缺口）。**首答 26.7% 接受为真实基线（用户拍板方向A）**，三根因写入文档：约束提取空白（改写 query 词表外）、评论商品 primary 低召回（真实评论压力测试 vs 锚点人工 gold）、negative 自动配对噪声（10 题泄漏）。**新增集 12 通道消融（_ablate_v3.py）**：A 唯一最优（E 灾难 4/75、F 融合全掉分、H3 21/75 是假象=+1 失真样本 q90 − 2 避雷 q94/q125）、D 持平、H≈H2。**12 通道最终定位（用户定稿）**：🟢上线 A tagfirst / 🟡兜底 D 双通路+H2 / 🔴实验禁 G·H3·F0-3·H·C·E + B 负向对照；**红线=语义模型绝不凌驾业务标签硬约束**。锚点零漂移：94.7%/0.553/0.889/105-105/8-8 逐数字复现（seed=42 幂等池重建）。详见 agent_design §8.17
- [x] **语义试探分支落地（2026-09-02 用户拍板，完成）**：reranker 置信度二次闸门——decide_ask 判 ask_all/ask_first 后不直接返回，先做一轮低成本语义试探（无约束 query → 语义通道 mixed 粗排全库 top-20 → bge-reranker 精排 → top-1 conf ≥ θ=0.05 → 走语义推荐 / < θ → 保持追问）。**根因**：语义通道是死代码（decide_ask L1064 直接 return，_retrieve 不执行），29 题词表外语义意图被误判无约束。**θ=0.05 区分度实证**：词表外语义题 conf 0.070-0.686 / 真模糊 q4/q5/q6 conf 0.001-0.008。**执行约束**：reranker 全程隔离 tagfirst / 语义产出带提示文案 / 完整走后置硬过滤（资损断言生效）/ 日志埋点。**验收门禁全过（实测）**：锚点 94.7%/0.553/105-105/资损 8-8 全数零漂移 / 新增轨 26.7% 基线不变 / q43/q64/q88/q115 等 14 条产出语义推荐（未碰 negative）/ q4/q5/q6 仍追问 / 差评轨资损 38/39 不恶化（q185 基线既有泄漏）。**实测误放 1→0（护栏，用户拍板选项 2）**：全量统计报表发现 q157（cruelty-free/vegan 硬约束）conf=0.060 临界放行混入池内 17-query negative 商品 → `_HARD_DECLARE` 强声明词（only buy/hard requirement/must have/no X at any stage/zero X）入口拦截保持追问——库外硬约束语义推荐无法硬保证。**风险**：兜底补偿非词表根治（B 方案留后续）、reranker 仅试探触发、θ 可配置留调参空间。详见 agent_design §8.18
- [x] **hard 轴三段降级：A 真雷硬踢 / B 无信息沉底 / C 卡片风险提示（2026-09-03 用户批准）**：敏感肌/痘痘肌 query 遇「商品客观缺该轴信息」不再一刀切 `-inf` 排除——命中该轴 consensus 缺陷证据（敏感肌→刺激 / 痘痘肌→闷痘）= A 真雷仍硬踢；无缺陷证据 = B 沉底（`HARD_SINK_SCORE=-1000`，非 -inf），仅覆盖品不足时补位；补位品卡片附「暂无该肤质用户反馈」风险提示 `risk_note`（C，双语）。落点：`ProductIndex.hard_verdict`（tag_score 与 recall_field 共用，防召回/打分层漂移）+ `_load_defect_map` 进程懒缓存 + agent._retrieve sink_pool + 前端 .risk。**实测零漂移**：锚点 94.7%/0.553/105-105/24-24/追问29.2% 逐数字复现、v3 41.4%/0.412/16-16/资损38-39 连跑两次确定性一致；真实库敏感肌覆盖 281/1090 → A/B 正常休眠（薄目录安全网），子库冒烟证明 sink 补位 + risk_note 通路。详见 agent_design §8.20
...
- [x] **对话记忆层 + 色号诊断续答系统化（2026-08-31 完成，实测七轮）**：用户实测诊断自测后答观察，Agent 不报色号结论反而推 3 款商品（用户原话：「我回答了具体情况你不应该告诉我我适合什么色号吗？」）。**根因系统化**（不是加关键词）= ①前端 context 只拼「原需求+回答」、不拼 **AI 上一条问了什么** → 闸门无法识别「这是回答诊断提问」；②无 shade_diagnosis 意图 + 会话状态，色号家族无跨轮记忆
  - **对话记忆层（数据层）**：前端每轮传「最近 8 轮**用户+AI 双方**」convo；后端 `_store_convo` 每轮压缩落盘（2026-09-03 热冷分离后改落冷文件 `user_convo.json`，拆出前是 `user_profiles.json.convo`）= `{orig, req 抽取约束, recent 最近6轮双方对话, diag_family, updated}`——**每用户专有 user_id + 每轮存储**，换浏览器重开也在。闸门/推荐器都从记忆取数，不再靠前端拼超长字符串
...
- [x] **企业级检索链路骨架（Phase-MVP，2026-09-01 完成）**：目标=参考达观搜推做企业级落地 AI 导购，用户定「先简单跑 MVP 看是否成功」，MVP 拆四件 ①②多路召回+路由分流（核心）③精排器接口 ①配置化 ④验证——全部落地：
  - **`scripts/recall_router.py`（多路召回 + 路由分流，核心）**：四路召回 `recall_field`（字段路=硬约束通过全集，等价 tag_score 硬排除）/ `recall_text`（BM25 top-K）/ `recall_hot`（heat top-K）/ `recall_vector`（bge 惰性加载，未加载自动返回 [] 降级）→ 并集去重成候选池；`has_struct` 动态路由（对齐 eval_dual_channel 的 D 通道口径：budget/hard/form/avoid/shade 或任意结构化约束或隐式意图 → **tagfirst**；无约束 → **语义通道** BM25+向量）。**锚点铁律**：锚点题全含结构化约束 → 全走 tagfirst；字段路=硬约束通过全集 → 并集排序与旧全库 tagfirst **字节级一致**（硬排除项本就 -inf 跳过，幸存项顺序不变，tuple 稳定排序）。**route_trace 随 record 落 harness_trace.jsonl**（channel + field/text/hot/vector/union 各多少 → 可观测：每轮走了哪个通道、各路召回多少）。已接线到 `agent._retrieve`：`self.recall_router.route_and_recall(req)` → `meta["route"]` → `_record["route"]`。诚实口径：1090 款小库字段路≈全库，多路召回是**骨架验证**，海量数据时字段路先粗筛、各路并集才是候选池——召回层在海量场景承重
  - **`scripts/ranker.py`（精排器接口 + 行为模型预留位）**：对齐 pangu 排序模块「粗排 → 精排」——`Ranker` ABC（`rank(candidates, req, meta)`）+ `ColdStartRanker`（现行唯一可用精排 = tagfirst，无行为信号的冷启动口径）+ `BehaviorRanker`（**预留位**：LambdaRank 双塔接口已定、特征清单已列——曝光/点击/转化/CTR/加购率/GMV/价格带拟合/复购热度，**未训练**（无 CTR/GMV 数据，局限1）；选中时安全降级冷启动 + 显式警告）。调用点 `_retrieve` 主链路 `self.ranker.rank(...)`——**换精排只改 `config.RANKER` 一行，接口与调用点不变 = 可演进架构的落点**
  - **`scripts/config.py`（配置化，对齐 pangu 方案配置/运营调优）**：可调参数收拢单一真相源——`HEAT_HI/MID`（热销分档）/ `DEFECT_CONSENSUS`（避雷 70%）/ `SESSION_MAX_QUERIES/LLM/WINDOW_SEC`（会话预算）/ `MAX_PROFILES`（画像上限）/ `SEM_WEIGHTS`（语义权重）/ `RECALL_TEXT/HOT/VEC_K`（召回 top-K）/ `RANKER`（精排器选择）；agent/harness/web_server/defect_consensus/eval_dual_channel/recall_router/ranker 七处改 import，值不变 → 可改、可审计、可回滚
  - **验收（锚点逐数字复现）**：首答 **18/19=94.7%** / NDCG@5 0.553 / 避雷 0.889 / **CONTRACT 105/105** / **资损 8/8** / 追问 29.2% / 降级 0% / 兜底 8.3%（eval_runner 全量 + eval_loss_risk 全过）；系统层 24 题 86-97ms（路由层零模型零 IO 开销）；`recall_router.py` 自测：结构化 → tagfirst / 无约束 → semantic，字段路 1090、向量未加载自动降级 0
  - **诚实边界**：①运行时无约束 query 会被 decide_ask 追问拦截（ask_all），语义通道主要服务 eval「what-if」与海量场景——MVP 证明的是**召回/路由骨架 + 无损性 + 可观测**，不是语义通道的线上收益；②`eval_compare.py`（二期 A/B）未重跑——它 re-hit DeepSeek API 且 LLM 非确定性，本 MVP 改动全在 `_retrieve` 路由/排序（A/B 意图决策下游），A 侧锚点 + CONTRACT 105/105 已全量复现，重跑只会用 LLM 抖动污染干净回归叙事
- [x] **数据看板（2026-09-01 完成，Phase-MVP 可观测收口）**：`scripts/dashboard.py` 读 `data/harness_trace.jsonl` 全链路埋点 → 聚合 → 生成自包含静态页 `data/dashboard.html`（双击浏览器打开，零第三方库，图表纯 CSS/SVG）。**统计口径对齐 harness._trace 字段**：检索耗时（avg/P50/P90/P95 + 直方图 + 慢查询 top-5）、**检索方式**（按条件硬筛 vs 语义匹配 + 各路平均召回数，route 自 2026-09-01 起埋点，旧行无字段单独标注）、**每轮怎么处理**（直接推荐/追问/软询问色号 + 正常/诚实兜底/全兜底 + 改写重试，状态胶囊）、**用户语言 & 识别方式**（中文/英文 + 规则直达/AI 识别，运营看 AI 成本）、**成本与安全护栏**（提问次数·AI 分析次数累计 + 自动降级会话数=budget.llm≥20 反推 + 医疗免责）、**最常被推荐的 10 款**（rec + products_clean.csv 标题映射）。**事件行**（gate_rejected/run_error）单独计拒绝/错误不进主统计。**设计语言：明亮风 SaaS 中后台**（用户固定记忆 bright-saas-admin-design：浅紫蓝渐变 `#EEF1FF→#FAFAFE` + 白色大圆角面板 24px 多重阴影 + 主色 `#4F6BFF` + 状态胶囊 + tabular-nums）。**运营友好（2026-09-01 用户「运营很难看懂」反馈）**：内部枚举全翻译成大白话（intent·none→规则直达、lang·zh→中文、tagfirst→按条件硬筛、LLM 触发→AI 深度分析、决策分布→每轮怎么处理）+ 每块灰字「这个指标看什么」，纯展示层改动零风险。验证：68 条主记录全解析、div 开闭平衡、无残留内部字段名。**项目叙事**：「埋点早已全落，只差可视化——检索耗时/检索方式/追问兜底降级率一张看板讲完企业级可观测，指标按运营语言呈现」
- [x] **行为精排演进位（2026-09-01 完成，文档级）**：`docs/enterprise_evolution.md` 演进路线图——把「接口已就位、数据来了就接」讲透。覆盖：现状冷启动（为什么 tagfirst：Amazon 静态快照无 CTR/GMV）、演进目标（LambdaRank 双塔：Query 塔/Item 塔/内积 + 在线 Item 塔预计算）、**特征清单**（ranker.BehaviorRanker 已列：曝光/点击/转化/CTR/加购率/GMV/价格带拟合/复购/差评规避后转化）、**数据补齐路径**（复用 harness_trace 扩展为「请求→曝光→点击→转化」行为漏斗 → 正负样本 → LambdaRank）、上线闭环（埋点→训练→shadow 小流量→持续重训，评测指标升级 CTR@k/GMV@k，内容锚点作回归红线）、项目叙事（接口在前数据在后 / 诚实边界 / 对齐 pangu 粗排精排）。**局限1 的答案** = 「精排接口已就位、特征已列、数据是唯一缺口，演进路径明确」
- [x] **存储接口抽象（2026-09-01 完成）**：`scripts/store.py`（对齐 pangu 存储层「接口在前、实现在后」）——`KVBackend` 抽象基类（get_all/save_all）+ `JsonKVBackend`（JSON 文件实现，保持现有格式）+ `RedisKVBackend`（**预留位**：接口同，真实部署传 redis 客户端即切换，零依赖环境构造显式 NotImplementedError 诚实声明）+ `ProfileStore`（画像高层 API：get/touch + MAX_PROFILES LRU 淘汰，把原 web_server._get_profile/_touch 函数体搬入封装）。**web_server 只做薄委托**：`_load_profiles/_save_profiles/_get_profile/_touch` 全委托 `_profile_store`（`ProfileStore(JsonKVBackend(PROFILE))`），**锁语义不变**（锁仍在 web 层持有，store 内不加锁避免重蹈非重入锁嵌套死锁坑）。验收：store.py 自测（临时路径读写+淘汰）+ web_server 隔离冒烟 4 项全过（handle_profile 写读 / handle_chat 英文全链路 / 生产 user_profiles.json 零污染 / Redis 预留位声明）。**换存储不换业务代码** = 讲「JSON 实现今天就能跑，Redis 部署只换构造一行」
- [x] **记忆层高并发演进第 1/3 段（2026-09-03 完成，用户选「拆独立冷文件」）**：① KVBackend 接口下沉 **per-key**（get/set/delete 主路径一人一条 + get_all/save_all 仅 LRU 淘汰/迁移等需全表遍历场景；JsonKVBackend 单文件=整表、per-key 底层整读写——诚实注释，demo 规模足够）；② 画像拆**热/冷双后端** `ProfileStore(hot, cold)`——热 = `user_profiles.json`（lang/skins/last_visit/created，每请求读），冷 = `user_convo.json`（convo 对话记忆独立文件、低频读、可丢，将来 Redis 下冷 key 可带 TTL）；**写对话记忆不再背着画像整表落盘**；LRU 淘汰热用户时同步清其冷 convo（不留孤儿）；旧画像内嵌 convo 启动时 `migrate()` 一次性拆到冷表（幂等，14 uids 实测 u_zncd1et9/u_test_concise convo 无损）。**write-behind 异步落盘 / 多实例一致性交给存储（Redis 原子 + TTL）/ uid 分片 = 演进第 2 段及之后，仅演进记录、不在 demo 落地**。验收：store.py 自测（热冷分离+迁移幂等+LRU 同步清冷）、真实副本迁移冒烟、web_server import 无副作用（migrate 只在 main()）、**锚点零漂移**：首答 94.7% / NDCG 0.553 / CONTRACT 105/105 / 全库避雷 24/24 逐数字复现。对外口径：「JSON 单文件今天就能跑（per-key 诚实标注底层整读写），Redis 部署只换构造一行且冷 key 自带 TTL」
- [x] **对已推荐商品的追问：新增 product_qa 意图（2026-09-01 完成，用户实测 badcase）**：推荐后问「这三款会不会卡粉/适合干皮吗/容不容易脱妆」这类商品级追问 → 新意图 product_qa（`_QA_PROD_REF`+`_QA_RISK`+`_QA_VERB`+`_QA_SUIT` 规则，插在 help 前）→ 绑定刚推荐商品真实数据（标签/评分/条数/价格/色号/差评主题）逐款回答；LLM 挂 → 确定性朋友式兜底 `_product_qa_fallback`（不崩、不反问已知）；`_req_meta` 补 CJK 规则层抽取（中文需求 ctx_line 真正带「肤质=混干+混油」→ 闸门全意图不再反问已知肤质）；help prompt 注入上轮推荐 + 硬约束不反问已知。**修掉三个用户抱怨**：逻辑链路不绑商品 / 反问「你手边哪几款」「什么肤质」/ 语气像教科书。锚点零回归：首答 94.7% / NDCG 0.553 / CONTRACT 105/105 / 全库避雷 24/24 / 资损 8/8（详见 agent_design §8.15）→ **回答精简化定稿（2026-09-01 用户「不错但太多」反馈）**：只改 `_generate_product_qa` prompt 为三段式硬结构——开头肤色判别≤2-3 行（仅问色号时） / 逐款每款 1-2 行（质地+妆效+适合谁 + 评论区声音/风险 + 关键提醒） / 总结 1-2 行（结论+收窄问），原硬约束全保留。真实两轮链路实测输出 395 字符（旧版 1000+）：开头 2 行肤色判别、Maybelline/Serious/HUDA 各 1-2 行绑定真实数据与评论区（「评论区没人提卡粉」「评论区就一条」）、总结 2 行含收窄问「偏自然色还是白皙？」。锚点 94.7%/105/105/8/8 逐数字复现零漂移（只动 LLM 文案，分类/检索/排序字节不变，详见 agent_design §8.16）
